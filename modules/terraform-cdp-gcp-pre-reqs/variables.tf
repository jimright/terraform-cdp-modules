...
variable "api_services_disable_on_destroy" {
  type = bool
...
variable "api_services_disable_dependent_services" {
  type = bool

  description = "Flag to control whether services that are enabled and which depend on the service being disabled should also be disabled when the resource is destroyed. Only relevant if api_services_disable_on_destroy is true."
