...
| <a name="input_cdp_delegated_subnet_names"></a> [cdp\_delegated\_subnet\_names](#input\_cdp\_delegated\_subnet\_names) | List of subnet names delegated for Flexible Servers. Only used if create\_vnet is false. | `list(any)` | `null` | no |
...
| <a name="input_skip_xaccount_app_data_source"></a> [skip\_xaccount\_app\_data\_source](#input\_skip\_xaccount\_app\_data\_source) | Skip reading Azure AD application details via data source. <br/>Set to true when the Terraform executor lacks Azure Entra ID read permissions.<br/>and all required app details are provided explicitly via variables.<br/>When true and using an existing app, you must provide existing\_xaccount\_app\_client\_id and existing\_xaccount\_app\_pword. | `bool` | `false` | no |
