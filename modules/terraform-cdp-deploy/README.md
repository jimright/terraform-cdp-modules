...
| <a name="input_aws_xaccount_skip_org_policy_decisions"></a> [aws\_xaccount\_skip\_org\_policy\_decisions](#input\_aws\_xaccount\_skip\_org\_policy\_decisions) | Flag to specify if the Cloudera Cross Account Credential should skip organization policy decisions. Only used for CDP deployment on AWS. | `bool` | `null` | no |
| <a name="input_aws_xaccount_verify_permissions"></a> [aws\_xaccount\_verify\_permissions](#input\_aws\_xaccount\_verify\_permissions) | Flag to specify if the Cloudera Cross Account Credential should verify permissions. Only used for CDP deployment on AWS. | `bool` | `null` | no |
...
| <a name="input_azure_cdp_resource_group_name"></a> [azure\_cdp\_resource\_group\_name](#input\_azure\_cdp\_resource\_group\_name) | Azure Resource Group name for CDP resources. Required for Cloudera on Azure deployment. | `string` | `null` | no |
...
| <a name="input_azure_create_private_endpoints"></a> [azure\_create\_private\_endpoints](#input\_azure\_create\_private\_endpoints) | Flag to specify that Azure Postgres will be configured with Private Endpoint and a Private DNS Zone. Mutually exclusive with delegated subnet input variables. | `bool` | `null` | no |
| <a name="input_azure_data_service_configurations"></a> [azure\_data\_service\_configurations](#input\_azure\_data\_service\_configurations) | Configuration for data services. Includes the user-assigned managed identity used by the AKS control plane and optionally the full Azure resource ID of an existing Private DNS zone used for the AKS. | <pre>object({<br/>    shared_managed_identity = string<br/>    aks_private_dns_zone_id = optional(string)<br/>  })</pre> | `null` | no |
...
| <a name="input_azure_datalake_database_type"></a> [azure\_datalake\_database\_type](#input\_azure\_datalake\_database\_type) | The Azure database type for the datalake. Valid values are FLEXIBLE\_SERVER or SINGLE\_SERVER. | `string` | `null` | no |
...
| <a name="input_azure_environment_availability_zones"></a> [azure\_environment\_availability\_zones](#input\_azure\_environment\_availability\_zones) | Set of availability zones to be used for deploying the environment. | `set(string)` | `null` | no |
| <a name="input_azure_environment_flexible_server_delegated_subnet_names"></a> [azure\_environment\_flexible\_server\_delegated\_subnet\_names](#input\_azure\_environment\_flexible\_server\_delegated\_subnet\_names) | List of Azure Subnet Names delegated for Private Flexible servers. This variable is mutually exclusive with azure\_create\_private\_endpoints. | `list(any)` | `null` | no |
...
| <a name="input_azure_network_resource_group_name"></a> [azure\_network\_resource\_group\_name](#input\_azure\_network\_resource\_group\_name) | Azure Resource Group name for Network resources. Required for Cloudera on Azure deployment. | `string` | `null` | no |
...
| <a name="input_cdp_xaccount_credential_name"></a> [cdp\_xaccount\_credential\_name](#input\_cdp\_xaccount\_credential\_name) | Name of the CDP Cross Account Credential. Defaults to '<env\_prefix>-xaccount-cred' if not specified. If create\_cdp\_credential is set to false then this should should be a valid pre-existing credential. | `string` | `null` | no |
...
| <a name="input_create_cdp_credential"></a> [create\_cdp\_credential](#input\_create\_cdp\_credential) | Flag to specify if the CDP Cross Account Credential should be created. If set to false then cdp\_xaccount\_credential\_name should be a valid pre-existing credential. | `bool` | `true` | no |
| <a name="input_custom_docker_registry"></a> [custom\_docker\_registry](#input\_custom\_docker\_registry) | The CRN of the desired custom docker registry for data services to be used. | <pre>object({<br/>    crn = string<br/>  })</pre> | `null` | no |
| <a name="input_datalake_architecture"></a> [datalake\_architecture](#input\_datalake\_architecture) | The CPU Architecture to be used for the Datalake instances. Only applicable for CDP deployment on AWS. | `string` | `null` | no |
...
| <a name="input_datalake_custom_instance_groups"></a> [datalake\_custom\_instance\_groups](#input\_datalake\_custom\_instance\_groups) | Custom properties to configure on an instance group level | <pre>set(<br/>    object({<br/>      instance_type = string,<br/>      name          = string<br/>    })<br/>  )</pre> | `null` | no |
| <a name="input_datalake_force_delete"></a> [datalake\_force\_delete](#input\_datalake\_force\_delete) | Flag to enable forced delete of datalake | `bool` | `null` | no |
...
| <a name="input_datalake_security_selinux"></a> [datalake\_security\_selinux](#input\_datalake\_security\_selinux) | Specify the SELinux configuration to be used for Datalake instances. Available values are PERMISSIVE or ENFORCING. | `string` | `null` | no |
...
| <a name="input_enable_ranger_rms"></a> [enable\_ranger\_rms](#input\_enable\_ranger\_rms) | Flag to enable Ranger Resource Mapping Server (RMS). Only applicable on Cloudera on AWS datalakes. | `bool` | `null` | no |
...
| <a name="input_environment_force_delete"></a> [environment\_force\_delete](#input\_environment\_force\_delete) | Flag to enable forced delete of environment | `bool` | `null` | no |
...
| <a name="input_environment_security_selinux"></a> [environment\_security\_selinux](#input\_environment\_security\_selinux) | Specify the SELinux configuration to be used. Available values are PERMISSIVE or ENFORCING. | `string` | `null` | no |
...
| <a name="input_freeipa_architecture"></a> [freeipa\_architecture](#input\_freeipa\_architecture) | The CPU Architecture to be used for the FreeIPA instances | `string` | `null` | no |
...
| <a name="input_gcp_availability_zones"></a> [gcp\_availability\_zones](#input\_gcp\_availability\_zones) | The zones of the environment in the given region. Specify multiple zones to enable cross-zone deployment of resources. | `list(string)` | `null` | no |
| <a name="input_gcp_cdp_gateway_subnet_names"></a> [gcp\_cdp\_gateway\_subnet\_names](#input\_gcp\_cdp\_gateway\_subnet\_names) | GCP Subnet Names for Endpoint Access Gateway. | `list(any)` | `null` | no |
